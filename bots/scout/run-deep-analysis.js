...
const modelRouter = (await import('/opt/mattermost/bots-v2/shared/model-router.js')).default;
const { publishPDF } = await import('/opt/mattermost/bots-v2/shared/publish-pdf.js');
...
Use specific numbers and data from the spreadsheet. Format tables properly.`;
  ...
  const resp1 = await modelRouter.generateResponse([{ role: 'user', content: prompt1 }], {
    model: 'opus',
    maxTokens: 16000,
    temperature: 0.3
  });
  chunks.push(resp1.content || resp1);
...
Use specific numbers. Create tables where appropriate.`;
  ...
  const resp2 = await modelRouter.generateResponse([{ role: 'user', content: prompt2 }], {
    model: 'opus',
    maxTokens: 16000,
    temperature: 0.3
  });
  chunks.push(resp2.content || resp2);
...
Based on the previous analysis, generate Part 3:
...
*Analysis completed using McKinsey 7S framework and HBS financial analysis standards.*`;
  ...
  const resp3 = await modelRouter.generateResponse([{ role: 'user', content: prompt3 }], {
    model: 'opus',
    maxTokens: 16000,
    temperature: 0.3
  });
  chunks.push(resp3.content || resp3);
...
async function main() {
  try {
    ...
    const statusPost = await postMessage(CHANNEL_ID, 
    ...
    console.log('4. Generating deep analysis (3 chunks)...');
    ...
    if (uploadResult.file_infos && uploadResult.file_infos.length > 0) {
      ...
      const finalPost = await fetch('https://opal.partnergroupconsulting.com/api/v4/posts', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${TOKEN}`,
          'Content-Type': 'application/json'
